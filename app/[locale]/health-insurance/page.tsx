...
import { CheckCircle, HeartPulse, Users, Baby, User } from 'lucide-react'
...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: 'meta.health' })
  return {
    title: t('title'),
    description: t('description'),
    alternates: buildAlternates(locale, '/health-insurance'),
    openGraph: {
      title: t('ogTitle'),
...
const planIcons = [User, Users, Baby] as const

export default function HealthInsurancePage() {
  const locale = useLocale()
  const providers = getProvidersByCategory('health', locale)
  const t = useTranslations('healthInsurance')
  const features = t.raw('features') as string[]
  const plans = [1, 2, 3].map((n, i) => ({
    icon: planIcons[i],
    title: t(`plan${n}Title`),
    description: t(`plan${n}Desc`),
    features: t.raw(`plan${n}Features`) as string[],
  }))
  const educSections = [
      ...
      <main id="main-content">
        ...
        <section className="relative min-h-[500px] overflow-hidden">
          <div
            ...
            style={{
              backgroundImage: `linear-gradient(to left, rgba(0,0,0,0.1), rgba(0,0,0,0.5)), url('https://images.unsplash.com/photo-1576091160399-112ba8d25d1d?w=1920&q=80')`,
            }}
          />
        ...
        {/* Providers offering health insurance */}
